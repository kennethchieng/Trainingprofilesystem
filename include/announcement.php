...
class Announcement {
	...
	function find_students($id="",$name=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE AnnouncementID = {$id} OR LNAME = '{$name}'");
	...
	function find_all_students($name=""){
		...
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblannouncement` WHERE `STUDUSERNAME` = '". $email ."' and `STUDPASS` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysqli_error());
		}
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb->loadSingleResult();
		 	$_SESSION['AnnouncementID']   	= $user_found->AnnouncementID;
	...
	function find_pass($id="",$pass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
			WHERE AnnouncementID = {$id} and STUDPASS = '{$pass}'");
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);
		return $row_count;
	}
